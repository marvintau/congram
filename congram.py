...
class CharColor:
    ...
    def __add__(self, inc):
        if type(inc) == tuple:
            if len(inc) == 2:
                return CharColor(self.fore + inc[0], self.back + self.fore + inc[1])
            elif len(inc) == 3:
...
class Canvas:
    ...
    def add_vertical_tick(self, tick_range, length, rep, off, anchor):
        for l in range(length+1):
            tick_char = u"├" if (l+off) % rep == 0 else u"│"
            self.elems.append(Rect(Pos(l, 0)+anchor, CharColor((255, 255, 255)), tick_char))

    def add_horizontal_tick(self, tick_range, length, rep, off, anchor):
        self.add_empty_line(anchor)
        for l in range(-2, length):
            tick_char = u"┴" if (l+off) % rep == 0 else u"─"
            self.elems.append(Rect(Pos(0, l)+anchor, CharColor((255, 255, 255)), tick_char))
